...
describe("Keccak 32bytes full hash test", function () {
	...
	describe("Keccak input: 4bytes, output: 32bytes, full hash test", function () {
		...
		describe("Keccak input: hello world (11 bytes), output: 32bytes, full hash test", function () {
			this.timeout(100000);

			let cir;
			before(async () => {
				cir = await c_tester(path.join(__dirname, "circuits", "keccak_88_256_test.circom"));
				await cir.loadConstraints();
				console.log("n_constraints", cir.constraints.length);
			});

			it("Keccak inputSize==88bits: 1 (manual testvector)", async () => {
			...
			it("Keccak256 inputSize==88bits, circom-js 1", async () => {
				let input, inputBits, expectedOut, witness, stateOut, stateOutBytes;
